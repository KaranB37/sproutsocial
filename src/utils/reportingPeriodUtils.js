...
export const getQuarterLabel = (quarter, fiscalYear) => {
  switch (quarter) {
    case 1: return `Q1 (Apr-Jun) FY${fiscalYear}-${String(fiscalYear + 1).slice(-2)}`;
    case 2: return `Q2 (Jul-Sep) FY${fiscalYear}-${String(fiscalYear + 1).slice(-2)}`;
    case 3: return `Q3 (Oct-Dec) FY${fiscalYear}-${String(fiscalYear + 1).slice(-2)}`;
    case 4: return `Q4 (Jan-Mar) FY${fiscalYear}-${String(fiscalYear + 1).slice(-2)}`;
    default: return `Unknown Quarter FY${fiscalYear}`;
  }
};
...
export const generateAllPeriods = (startDate, endDate, reportingPeriod) => {
  ...
  // For yearly reporting
    ...
    const startFiscalYear = startDate.getMonth() >= 3 ? 
      startDate.getFullYear() : 
      startDate.getFullYear() - 1;
      
    const endFiscalYear = endDate.getMonth() >= 3 ? 
      endDate.getFullYear() : 
 ...
 */
export const groupDataByReportingPeriod = (data, reportingPeriod, startDate, endDate) => {
  ...
  const result = Array.from(groupedData.values()).map(entry => {
    ...
    // For example, followers count should be averaged for the period
